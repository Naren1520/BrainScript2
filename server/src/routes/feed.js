...
// Endpoint to fetch videos from a specific YouTube channel using YouTube API
router.get("/channel", async (req, res) => {
  try {
    const { channelId = "", limit = 6 } = req.query;
    ...
    const videosRes = await fetch(
      `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet,contentDetails&playlistId=${uploadsPlaylistId}&maxResults=${limitNum}&key=${apiKey}`
    );
    ...
    return res.json({
      success: true,
      videos,
      total: videosData.items.length,
      returned: videos.length,
    });
  } catch (error) {
    console.error("Error fetching channel videos from YouTube API:", error);
    res.status(500).json({
      success: false,
