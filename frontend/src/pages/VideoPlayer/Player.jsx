...
const Player = () => {
  ...
  const [viewMode, setViewMode] = useState("notes"); // notes | transcript | summary | quiz
  ...
  const fetchTranscriptForActive = useCallback(
    async (opts = {}) => {
      ...
      try {
        ...
        try {
          const videoData = videos[activeVideoId] || {};
          await fetch(`${BASE_URL}/api/user/save-transcript`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
            body: JSON.stringify({
              videoId: activeVideoId,
              videoTitle: videoData.title || "Video Transcript",
              transcript: data.transcript || "",
              language: opts.lang || "en",
            }),
          }).catch((err) => console.error("Failed to auto-save transcript:", err));
        } catch (err) {
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      try {
        const videoData = videos[activeVideoId] || {};
        await fetch(`${BASE_URL}/api/user/save-summary`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
            videoId: activeVideoId,
            videoTitle: videoData.title || "Video Summary",
            summary: data.summary,
          }),
        });
      } catch (saveErr) {
        console.error("Failed to auto-save summary:", saveErr);
      }
    } catch (e) {
